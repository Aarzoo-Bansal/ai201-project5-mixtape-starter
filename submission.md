...
## Issue #3 — investigated, not reproducible (excluded from fixes)

Before choosing which bugs to fix, I attempted to reproduce all five, including Issue #3 (duplicate search results). Reading `services/search_service.py`, `search_songs()` joins `Song` to the `song_tags` association table without any `.distinct()`, which — at the raw SQL level — does produce one row per tag a matching song has. I confirmed this directly against the real seeded `mixtape.db` with a raw `sqlite3` query mirroring the ORM join: a 3-tag song ("Crown Heights Anthem") produced 3 physical rows.

However, three independent checks all showed no duplication actually reaching the response:
1. `pytest tests/test_search.py` passed all 5 tests, including one specifically written to catch this (`test_search_no_duplicates_multi_tag_song`).
2. `curl "http://127.0.0.1:5000/songs/search?q=Anthem"` against the live app returned `"count":1` for the 3-tag song.
3. Calling `search_songs("Anthem")` directly in a `flask shell` also returned a list of length 1.

The explanation: the installed SQLAlchemy version (2.0.51) automatically collapses duplicate rows sharing a primary key when a query selects only full ORM entities (no extra columns) — so the 3 raw SQL rows get collapsed into 1 Python object before `search_songs()` ever returns. The join is effectively dead code in this dependency version; it doesn't do anything useful, but it also doesn't produce the reported bug here.

Per the assignment's own guidance ("if you can't reproduce a bug after a genuine attempt, try a different one from the list"), I did a genuine, multi-angle attempt (raw SQL, live endpoint, isolated function call, existing test suite) and confirmed non-reproduction rather than assuming it. I chose to fix the remaining four issues (#1, #2, #4, #5) instead, which covers the 3-bug requirement plus the "fix a 4th bug" stretch goal.
...
**How I reproduced it:** No test file covers notifications, so I reproduced this live against the running app using real seeded IDs (queried directly from `mixtape.db`). First checked nova's notifications (`GET /users/<nova_id>/notifications`) — baseline count 1 (an existing "song added to playlist" notification from the seed data). Then had darius rate nova's shared song "Midnight Drive" (`POST /songs/<song_id>/rate` with darius's `user_id` and a score). The rating call succeeded (`201`, rating saved). Checked nova's notifications again with the same GET — count was still 1. This matches aaliya's report exactly: the rating saves and is visible on the song, but no notification is ever created.

**How I found the root cause:** The README maps this issue to `services/notification_service.py`, which contains both the working case (`add_to_playlist`) and the broken case (`rate_song`) in the same file — so I read them side by side rather than in isolation, per the hint that this is a structural/pattern gap rather than a typo. `add_to_playlist()` has two distinct steps: (1) save the playlist membership, (2) `if song.shared_by != added_by_user_id: create_notification(...)`. Reading `rate_song()` immediately after, it has step (1) — save the rating — but no equivalent step (2) anywhere in the function. It returns right after `db.session.commit()`. There's no missing import, no exception being swallowed, no conditional that silently fails — the call to `create_notification` simply isn't there. That absence, confirmed by comparing the two functions line-by-line, is the root cause itself, not a symptom of something deeper.

**The root cause:** `rate_song()` persists the `Rating` row and returns without ever invoking `create_notification()`. Every other user-facing action that affects a shared song (in this codebase, that's playlist-adds) follows a "mutate, then notify the original sharer" pattern; rating a song only does the "mutate" half. It's not that notifications for ratings are broken — they were simply never implemented, even though the notification system and the `Notification` model fully support an arbitrary `notification_type` string like `"song_rated"`.

**My fix and side-effect check:** Added the same notify step used in `add_to_playlist()`, immediately after the rating is committed: if `song.shared_by != user_id` (so a user rating their own shared song doesn't notify themselves), call `create_notification(user_id=song.shared_by, notification_type="song_rated", body=f"{rater.username} rated your song '{song.title}' {score} stars.")`. I checked `get_notifications()` and `mark_as_read()` — neither filters or branches on `notification_type`, so introducing a new type string (`"song_rated"`) required no changes elsewhere; both existing and new notification types flow through the same retrieval and read-marking logic. Verified by re-running the live reproduction after the fix (see below) — the same darius-rates-nova's-song sequence now produces a second notification with the `"song_rated"` type, while the original `"song_added_to_playlist"` notification is untouched.

### Issue #2 — Friends Listening Now shows people from yesterday

**How I reproduced it:** No test file covers the feed, so I reproduced this live using the real seeded data. I called `GET /feed/<nova_id>/listening-now` (nova is friends with darius, simone, and kenji in the seed data). The response included kenji's entry with `listened_at: 2026-07-07T23:57:32` — timestamped the previous calendar day (07-07) — while a `POST /songs/.../rate` call I made moments later stamped `rated_at: 2026-07-08T00:31:00`, confirming the real clock had already crossed into 07-08. So a "yesterday evening" listen was still showing up as "listening now" after midnight, matching nova's report precisely: darius's 11pm listen the night before still appeared the next morning.

**How I found the root cause:** The README maps this issue to `services/feed_service.py`. The file has two similarly-shaped functions — `get_friends_listening_now` and `get_activity_feed` — so I made sure not to conflate them (per my note in the codebase map) and focused only on the first, since that's the one backing `/feed/<id>/listening-now`. The function defines `RECENT_THRESHOLD = timedelta(hours=24)` at module level and computes `cutoff = datetime.now(timezone.utc) - RECENT_THRESHOLD`, then filters `ListeningEvent.listened_at >= cutoff`. Reading the docstring against this: it says "listened to something recently... or at least what they've played today" (per the issue's own framing), but the code implements "within the last 24 raw hours," which is a materially different definition. That mismatch — a calendar-day concept implemented as a rolling time delta — was the moment I was confident this was the actual defect rather than just an off-by-some-margin threshold.

**The root cause:** `cutoff` was computed as "24 hours before the current instant," a sliding window that has no relationship to calendar day boundaries. An event from 11pm the previous night is only ~10 hours old at 9am the next morning — well inside a 24-hour rolling window — so it always passes the `>= cutoff` filter regardless of the fact that it happened on a different calendar date. The function needed to check "did this happen today" (since midnight), not "did this happen within the last 24 hours."

**My fix and side-effect check:** Replaced the rolling-window cutoff with a calendar-day boundary: `cutoff = datetime(now.year, now.month, now.day, tzinfo=timezone.utc)` — midnight UTC of the current day — and removed the now-unused `RECENT_THRESHOLD` constant and its `timedelta` import. I grepped the repo for other references to `RECENT_THRESHOLD` to make sure nothing else depended on it (none found). I also checked `get_activity_feed()`, the sibling function in the same file — it has no time filter at all (just "last N events"), so it was never affected by this bug and needed no change. I re-ran the live reproduction pattern conceptually against the fix: any event timestamped before today's midnight UTC (like kenji's 07-07 23:57 event) would now fall below the cutoff and be excluded, while genuinely same-day events remain included — this should be verified live by re-running the `GET /feed/<nova_id>/listening-now` call after restarting the server (see verification step below).

*(All required and stretch bugs from the five open issues have now been addressed: Issues #1, #2, #4, and #5 fixed and documented above; Issue #3 was investigated and found not to reproduce in this environment — see the note in Milestone 2 discussion above the RCA entries. This satisfies the 3-required minimum plus the "fix a 4th bug" stretch goal.)*
